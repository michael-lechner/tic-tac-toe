...
class Main(cmd.Cmd):
    intro = '\nwelcome to tic tac toe! you can enter a move at the prompt for a given symbol using x,y ie: move 0,3\n'
    symbol = 'X'
    ...
    def is_board_full(self):
        return len(filter(lambda row: any(cell is None for cell in row), self.board)) == 0

    @staticmethod
    def does_sequence_match(sequence):
        if sequence[0] is None: return False
    ...
    def print_board(board):
        print '\n'
        for row in board:
            print ' | '.join(map(lambda x: x if x is not None else ' ', row))
        print '\n'
